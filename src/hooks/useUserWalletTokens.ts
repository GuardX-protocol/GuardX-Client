...
export const useWalletTokens = (chainId: number) => {
  ...
  const getTokenList = useCallback((chainId: number): TokenInfo[] => {
  ...
  const fetchAllTokens = useCallback(async () => {
    ...
    try {
      ...
      const tokenPromises = tokenList.map((token: TokenInfo) =>
      ...
      tokenResults.forEach((result: PromiseSettledResult<WalletToken | null>, index: number) => {
