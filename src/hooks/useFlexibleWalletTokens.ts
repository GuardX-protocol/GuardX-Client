...
export interface FlexibleWalletToken {
  address: string;
  symbol: string;
  name: string;
  decimals: number;
...
export const useFlexibleWalletTokens = (chainId: number, walletAddress?: string) => {
  const [tokens, setTokens] = useState<FlexibleWalletToken[]>([]);
  ...
  const fetchTokenBalance = async (
    ...
    tokenAddress: string,
    tokenInfo: any
  ): Promise<FlexibleWalletToken | null> => {
    try {
  ...
  const fetchAllTokens = useCallback(async () => {
    ...
    try {
      ...
      const validTokens: FlexibleWalletToken[] = [];
  ...
  }, [walletAddress, chainId, getTokenList]);

  useEffect(() => {
    if (walletAddress) {
      fetchAllTokens();
    }
  }, [fetchAllTokens, walletAddress]);
