...
import { MultiTokenVaultABI, ERC20ABI } from "@/config/abis/multiTokenVault";
import useVaultBalances from "@/hooks/useVaultBalances";

const MultiTokenDepositForm: React.FC = () => {
  ...
  const {
    write: depositERC20,
    data: erc20DepositTx,
    isLoading: isErc20DepositLoading,
  } = useContractWrite({
    address: VAULT_CONTRACTS.MULTI_TOKEN_VAULT_ADDRESS,
    abi: MultiTokenVaultABI,
  ...
  const {
    write: withdrawETH,
    data: ethWithdrawTx,
    isLoading: isEthWithdrawLoading,
  } = useContractWrite({
    address: VAULT_CONTRACTS.MULTI_TOKEN_VAULT_ADDRESS,
    abi: MultiTokenVaultABI,
  ...
  const {
    write: withdrawERC20,
    data: erc20WithdrawTx,
    isLoading: isErc20WithdrawLoading,
  } = useContractWrite({
    address: VAULT_CONTRACTS.MULTI_TOKEN_VAULT_ADDRESS,
    abi: MultiTokenVaultABI,
  ...
  const { data: allowance, refetch: refetchAllowance } = useContractRead({
    ...
    args: [address!, VAULT_CONTRACTS.MULTI_TOKEN_VAULT_ADDRESS],
    enabled:
      !!selectedToken &&
      !!address &&
      selectedToken.address !== "0x0000000000000000000000000000000000000000" &&
  ...
  // Get vault balances for withdrawal
  const { assets: vaultAssets, refetchAll: refetchVaultBalances } =
    useVaultBalances(address as `0x${string}`);
  // Transaction receipts for deposits
  const { isLoading: isDepositConfirming, isSuccess: isDepositSuccess } =
    useWaitForTransaction({
      hash: ethDepositTx?.hash || erc20DepositTx?.hash,
      onSuccess: () => {
        setAmount("");
  ...
  const { isLoading: isWithdrawConfirming, isSuccess: isWithdrawSuccess } =
    useWaitForTransaction({
      hash: ethWithdrawTx?.hash || erc20WithdrawTx?.hash,
      onSuccess: () => {
        setAmount("");
  ...
  const currentBalance = useMemo(() => {
    if (mode === "deposit") {
      ...
      // For withdrawals, get balance from vault
      const vaultAsset = vaultAssets.find(
        (asset) =>
  ...
  }, [mode, isETH, ethBalance, tokenBalance, vaultAssets, selectedToken]);
  ...
  const handleApproval = useCallback(async () => {
    ...
    try {
      ...
      approveToken?.({
        args: [VAULT_CONTRACTS.MULTI_TOKEN_VAULT_ADDRESS, parsedAmount],
      });
    } catch (error) {
  ...
  }, [isETH, address, selectedToken, approveToken, parsedAmount]);
  ...
  // Remove the useEffect hooks since we handle success/error in useWaitForTransaction
  // Reset form after successful deposit and approval are now handled in the hooks above
  // Hooks above handle success/error and reset form state.
  ...
  const isLoading =
    ...
    isEthDepositLoading ||
    isErc20DepositLoading ||
    isEthWithdrawLoading ||
    isErc20WithdrawLoading ||
    isApprovalLoading;
  if (!isConnected) {
    return (
  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {/* Transaction Status */}
        {(ethDepositTx?.hash || erc20DepositTx?.hash) && (
...
export default MultiTokenDepositForm;
