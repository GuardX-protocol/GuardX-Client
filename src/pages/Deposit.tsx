...
import MultiTokenDepositForm from "@/components/deposit/MultiTokenDepositForm";
...
const Deposit: React.FC = () => {
  ...
  // Get portfolio data from contract (legacy)
  const { portfolio, isLoading: portfolioLoading } = usePortfolioData(address);

  // Get vault balances from MultiTokenVault
  const {
    assets: vaultAssets,
  ...
  const totalValue = portfolioData
    ? Number(
        formatUnits(
          BigInt(portfolioData.totalValue || portfolioData[1] || 0),
          18
        )
      )
    : 0;
